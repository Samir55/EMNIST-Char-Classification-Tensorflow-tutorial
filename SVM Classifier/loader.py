...
PATH = '../data/small_symbols_dataset/'
NUM_CLASSES = 32

char_map = {}

def save_dict(filename, c_map):
...
def add_chars(training_set, training_results, test_set, test_results):
  ...
  for img in listdir((PATH + 'ls')):
    ...
    if (i < (5.0/6) * cnt):
      ...
      #a[char_map[94]] = 1
      training_results.append(char_map[94])
    else:
      # Append to testset
      test_set.append(img)
      #a = np.zeros(NUM_CLASSES)
      #a[char_map[94]] = 1
      test_results.append(char_map[94])
  ...
  for img in listdir((PATH + 'dash')):
    ...
    if (i < (5.0/6) * cnt):
      ...
      #a = np.zeros(NUM_CLASSES)
      #a[char_map[95]] = 1
      training_results.append(char_map[95])
    else:
      # Append to testset
      test_set.append(img)
      #a = np.zeros(NUM_CLASSES)
      #a[char_map[95]] = 1
      test_results.append(char_map[95])
    i+=1
...
def read_data_sets(train_dir = "",
  ...
  save_dict('char_map.txt', inv_char_map) 

  return (train, validation, test)


def load_dataset(train_dir='MNIST-data'):
